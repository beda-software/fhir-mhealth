...
class KeychainStorage: NSObject {
  @objc func store(_ key: String, value: String,
  ...
  @objc func retrieve(_ key: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    DispatchQueue.global(qos: .userInitiated).async {
      let query: [String: Any] = [
        kSecClass.string: kSecClassGenericPassword,
        kSecAttrAccount.string: key,
  ...
  @objc func remove(_ key: String, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    DispatchQueue.global(qos: .userInitiated).async {
      let query: [String: Any] = [
        kSecClass.string: kSecClassGenericPassword,
        kSecAttrAccount.string: key,
